...
void InsertionTrace(std::vector<Clave>& vec, unsigned size) {
  ...
  for (unsigned int i = 1; i < size; i++) {
	...
	while (j >= 0 && vec[j] > clave) {
	  std::cout << vec[j] << " > " << clave.key << std::endl;
	  vec[j + 1] = vec[j];
	  j--;
	}
	std::cout << vec[j + 1] << " cambiado con " << clave.key << std::endl; 
	vec[j + 1] = clave;
...
void DeltaSortTrace(std::vector<Clave>& vec, int d, int n ) {
  ...
  for (int i = d; i < n; i++){
    ...
    if(vec[j] == clave)
        std::cout << "No se realiza desplazamiento ya que " << clave.key << " no es menor que " << vec[j-d] << std::endl;
    vec[j] = clave ;
    PrintVector(vec, 0, vec.size()-1);
  }
}
